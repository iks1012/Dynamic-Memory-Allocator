...
Test(test1, syn1, .init = bud_mem_init, .fini = bud_mem_fini, .timeout = 5) {
    void *x = bud_malloc(sizeof(double) * 4);
    void *header = x - sizeof(bud_header);
    cr_assert_neq(isValidPtr(header), 0,
		  "pointer is not valid");
}
Test(test1, syn2, .init = bud_mem_init, .fini = bud_mem_fini, .timeout = 5) {
	void *x = bud_malloc(sizeof(double) * 4); // 32 -> 64
    void *y = bud_realloc(x, sizeof(int));
    cr_assert_neq(isValidPtr(y - sizeof(bud_header)), 0,
		  "pointer is not valid");
}
Test(test1, syn3, .init = bud_mem_init, .fini = bud_mem_fini, .timeout = 5) {void *x = bud_malloc(sizeof(double) * 4); // 32 -> 64
    void *y = bud_realloc(x, sizeof(int));
    cr_assert_neq(isValidPtr(y - sizeof(bud_header)), 0,
		  "pointer is not valid");}
Test(test1, syn4, .init = bud_mem_init, .fini = bud_mem_fini, .timeout = 5) {void *x = bud_malloc(sizeof(double) * 4); // 32 -> 64
    void *y = bud_realloc(x, sizeof(int));
    cr_assert_neq(isValidPtr(y - sizeof(bud_header)), 0,
		  "pointer is not valid");}
Test(test1, syn5, .init = bud_mem_init, .fini = bud_mem_fini, .timeout = 5) {void *x = bud_malloc(sizeof(double) * 4); // 32 -> 64
    void *y = bud_realloc(x, sizeof(int));
    cr_assert_neq(isValidPtr(y - sizeof(bud_header)), 0,
		  "pointer is not valid");}
